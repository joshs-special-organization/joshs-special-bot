...
export function dateifyRule(data: any) {
	return ({ ...data, createdAt: new Date(data.createdAt) }) as Rule
}

export async function isRuleApplicable(rule: Rule, message: Message) {
	if (rule.forId != message.author.id) return false;
	if (await isRuleOutOfDate(rule)) return false;
	if (message.author.bot) return false;

	return transliterate(message.content.toLowerCase()).replace(/ /g, "").includes(rule.phrase.toLowerCase().replace(/ /g, ""))
}

export async function getRulesMadeFor(userId: string) {
	// let { data, error } = await supabase.from("rules").select().eq("for_id", userId)
	let data = await prisma.muteRule.findMany({ where: { forId: userId } })

	if (!data || data.length == 0)
		return { error: "No data was returned from the db request" }

	let rules: Rule[] = data.map(dateifyRule) // Convert created_at attribute to a Date
...
export async function getRuleMadeBy(userId: string) {
	...
	let data = await prisma.muteRule.findFirst({ where: { creatorId: userId } })
...
export async function removeRuleMadeBy(userId: string) {
	...
	let { error: ruleError, rule } = await getRuleMadeBy(userId)
	if (!rule || ruleError)
		return { error: "You currently have no rule in place" };

	return dateifyRule(await prisma.muteRule.delete({ where: { creatorId: userId } }))
...
export async function userHasRuleCreated(userId: string) {
	let { rule, error } = await getRuleMadeBy(userId)
