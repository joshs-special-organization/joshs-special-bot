...
export async function execute(interaction: CommandInteraction) {
	const { rule, error: _ } = await getRuleMadeBy(interaction.user.id, interaction.guildId as string)
	const { error } = await removeRuleMadeBy(interaction.user.id, interaction.guildId as string)
