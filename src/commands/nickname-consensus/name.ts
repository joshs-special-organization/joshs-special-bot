...
import {
    ...
    getIntendedNickname,
    setIntendedNickname,
} from '../../functions/nickname-consensus'
...
export async function execute(interaction: ChatInputCommandInteraction) {
    const mentionedUser = interaction.options.getMember('user') as GuildMember
    ...
    if (['set', 'reset'].includes(interaction.options.getSubcommand())) {
        if (mentionedUser.user.id == interaction.user.id) {
            return interaction.reply({
                content: 'Only other users can alter your nickname',
                ephemeral: true,
            })
        }

        oldNickname =
            mentionedUser.nickname || getFallbackNickname(mentionedUser)
    }
    ...
    switch (interaction.options.getSubcommand()) {
        case 'set':
            ...
            if ((await getIntendedNickname(mentionedUser)) == intendedNickname)
                return interaction.reply({
                    content: `<@${mentionedUser.id}> already has that nickname`,
                    ephemeral: true,
                })
            ...
            return interaction.reply({
                content: `<@${mentionedUser.id}>: Your name has been updated from **${oldNickname}** to **${intendedNickname}**`,
            })

        case 'reset':
            if (!(await getIntendedNickname(mentionedUser)))
                return interaction.reply({
                    content: `<@${mentionedUser.id}> currently has no nickname`,
                    ephemeral: true,
                })
            ...
            await setIntendedNickname(mentionedUser)
            return interaction.reply({
                content: `Reset <@${mentionedUser.id}> from **${oldNickname}** to **${mentionedUser.user.username}**`,
            })

        case 'info':
            if (!(await getIntendedNickname(member))) {
                return interaction.reply({
                    content: 'No one has set your nickname, yet...',
                    ephemeral: true,
                })
            }
            ...
            return interaction.reply({
                content: `Your current nickname was set by <@${record?.setByUserId}>`,
                ephemeral: true,
            })
    }
